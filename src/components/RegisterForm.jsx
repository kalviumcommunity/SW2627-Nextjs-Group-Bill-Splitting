...
export default function RegisterForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setServerStatus({
        type: "success",
        message: `Account initialized for ${payload.fullName}. 6-digit OTP dispatched to ${payload.email}.`,
      });
      setOtpSent(true);
    } catch (err) {
      setServerStatus({
        ...
        <div className="w-full max-w-[440px] bg-white rounded-3xl p-8 sm:p-10 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.06),0_0_1px_1px_rgba(0,0,0,0.03)] border border-[#ebe7dc] transition-all duration-300">
          ...
          {!otpSent ? (
            ...
            <div className="text-center py-4 space-y-6 animate-fadeIn">
              <div className="w-14 h-14 rounded-2xl bg-[#0f0f12] text-white flex items-center justify-center mx-auto text-xl shadow-lg">
                ✉
              </div>
              <div>
                <h3 className="font-serif-luxury text-2xl font-bold text-[#0f0f12]">
                  Verify Email
                </h3>
                <p className="text-xs text-[#6c6a75] mt-1.5 leading-relaxed">
                  We've sent a 6-digit security code to
                  <br />
                  <strong className="text-[#0f0f12] font-semibold">{formData.email}</strong>
                </p>
              </div>
              ...
              <div className="flex justify-center gap-2 sm:gap-2.5 my-4">
                {otpValues.map((val, idx) => (
                  <input
                    ...
                    className="w-10 sm:w-11 h-12 sm:h-14 text-center text-lg sm:text-xl font-bold rounded-xl border border-[#d6cfbe] bg-[#f8f6f0] focus:border-[#0f0f12] focus:bg-white focus:outline-none transition-all shadow-xs"
                    placeholder="•"
                  />
                ))}
              </div>
              ...
              <button
                type="button"
                className="w-full bg-[#0f0f12] text-white py-3.5 px-4 rounded-xl font-bold text-xs tracking-widest uppercase hover:bg-[#25242b] transition-all cursor-pointer shadow-md"
              >
                VERIFY &amp; ACTIVATE →
              </button>
              ...
              <button
                ...
                className="text-xs text-[#787582] hover:text-[#0f0f12] underline cursor-pointer transition-colors"
              >
